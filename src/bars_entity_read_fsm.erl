...
start(VNodeInfo, Op, User, Val) ->
    ReqID = mk_reqid(),
    bars_entity_read_fsm_sup:start_read_fsm(
...
-spec merge([vnode_reply()]) -> bars_obj() | not_found.
merge(Replies) ->
    Objs = [Obj || {_,Obj} <- Replies],
    bars_obj:merge(bars_entity_read_fsm, Objs).
...
%% @pure
different(A) -> fun(B) -> not bars_obj:equal(A,B) end.

%% @impure
%%
%% @doc Repair any vnodes that do not have the correct object.
-spec repair(atom(), string(), bars_obj(), [vnode_reply()]) -> io.
repair(_, _, _, []) -> io;

repair(VNode, StatName, MObj, [{IdxNode,Obj}|T]) ->
    case bars_obj:equal(MObj, Obj) of
